...
namespace Nyahoon {
	// Windows Store .NET Core doesn't have System.Threading.Thread. Instead, use Windows.System.Threading.ThreadPool.
#if UNITY_2019_1_OR_NEWER || !NETFX_CORE
	...
	public class ThreadPool {
		...
		private struct Job : IJob
		{
			...
			{
				m_func = System.Runtime.InteropServices.GCHandle.Alloc(callback);
				m_arg = System.Runtime.InteropServices.GCHandle.Alloc(state);
			}
			public void Execute()
			{
				WaitCallback callback = (WaitCallback)m_func.Target;
				object arg = m_arg.Target;
				m_func.Free();
				m_arg.Free();
				callback(arg);
			}
			private System.Runtime.InteropServices.GCHandle m_func;
			private System.Runtime.InteropServices.GCHandle m_arg;
		}
		...
		{
			Job job = new Job(callback, state);
			var jobHandle = job.Schedule();
			// make sure that Unity Engine starts job threads.
		...
		private volatile int m_numTasks;
