...
namespace FastShadowReceiver {
	public class TerrainMeshTree : MeshTreeBase {
		...
		const int CHILD_INDEX_OF_LEAF_NODE = -1;
#if UNITY_2019_3_OR_NEWER
		const int CHILD_INDEX_OF_HOLE_NODE = -2;
#endif
		...
		{
			int resolution = m_terrainData.heightmapResolution;
			m_heightMapWidth = resolution - 1;
			m_heightMap = m_terrainData.GetHeights(0, 0, resolution, resolution);
#if UNITY_2019_3_OR_NEWER
			m_holeMap = m_terrainData.GetHoles(0, 0, resolution-1, resolution-1);
#endif
...
#if UNITY_2019_3_OR_NEWER
			terrainSearch.Finalize(m_heightMap, m_scale.x, m_scale.z, m_heightMapWidth, m_holeMap);
#else
			terrainSearch.Finalize(m_heightMap, m_scale.x, m_scale.z, m_heightMapWidth);
#endif
		}
...
#if UNITY_2019_3_OR_NEWER
			else if (node.m_childIndex == CHILD_INDEX_OF_HOLE_NODE)
			{
				return;
			}
#endif
		...
		private bool Raycast(MeshTreeRaycast raycast, ref TreeNode node, Vector3 extents, int posX, int posZ, int width, ref MeshTreeRaycast.TemporaryParam param, uint order)
		{
			...
			float distance;
#if UNITY_2019_3_OR_NEWER
			if (node.m_childIndex == CHILD_INDEX_OF_HOLE_NODE)
			{
				return false;
			}
#endif
		...
		private void BuildMeshTree(List<TreeNode> nodeList, ref TreeNode parentNode, int posX, int posZ, int width)
		{
			if (width <= 2) {
#if UNITY_2019_3_OR_NEWER
				if (UpdateNodeBounds(ref parentNode, posX, posZ, width))
#else
				UpdateNodeBounds(ref parentNode, posX, posZ, width);
#endif
				{
					parentNode.m_childIndex = CHILD_INDEX_OF_LEAF_NODE;
				}
#if UNITY_2019_3_OR_NEWER
				else
				{
					parentNode.m_childIndex = CHILD_INDEX_OF_HOLE_NODE;
				}
#endif
#if UNITY_EDITOR
			...
			if (child0.m_childIndex != CHILD_INDEX_OF_HOLE_NODE)
			{
				hasChildren = true;
			}
			if (child1.m_childIndex != CHILD_INDEX_OF_HOLE_NODE)
			{
				if (hasChildren)
				{
				...
				{
					minY = child1.m_yCenter - child1.m_yExtent;
					maxY = child1.m_yCenter + child1.m_yExtent;
				}
				hasChildren = true;
			}
			if (child2.m_childIndex != CHILD_INDEX_OF_HOLE_NODE)
			{
				if (hasChildren)
				{
				...
				{
					minY = child2.m_yCenter - child2.m_yExtent;
					maxY = child2.m_yCenter + child2.m_yExtent;
				}
				hasChildren = true;
			}
			if (child3.m_childIndex != CHILD_INDEX_OF_HOLE_NODE)
			{
				if (hasChildren)
				{
				...
				{
					minY = child3.m_yCenter - child3.m_yExtent;
					maxY = child3.m_yCenter + child3.m_yExtent;
				}
				hasChildren = true;
			}
#else
		...
		private bool UpdateNodeBounds(ref TreeNode node, int posX, int posZ, int width)
		{
			...
			{
				for (int z = posZ; z < endZ; ++z)
				{
					if (m_holeMap[z, x])
					{
						float h0 = m_heightMap[z, x];
						float h1 = m_heightMap[z + 1, x];
						float h2 = m_heightMap[z, x + 1];
						float h3 = m_heightMap[z + 1, x + 1];
						float max0 = Mathf.Max(h0, h1);
						float max1 = Mathf.Max(h2, h3);
						float max = Mathf.Max(max0, max1);
						float min0 = Mathf.Min(h0, h1);
						float min1 = Mathf.Min(h2, h3);
						float min = Mathf.Min(min0, min1);
						if (cellCount == 0)
						{
							maxY = max;
							minY = min;
						...
						{
							maxY = Mathf.Max(maxY, max);
							minY = Mathf.Min(minY, min);
						}
						++cellCount;
					}
				}
