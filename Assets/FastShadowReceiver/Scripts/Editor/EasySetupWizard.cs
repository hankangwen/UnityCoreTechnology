...
namespace FastShadowReceiver.Editor {
	public class EasySetupWizard : EditorWindow {
				...
				if (meshTree is TerrainMeshTree) {
					...
					if (srcMesh is Mesh) {
						...
						if (srcMesh != null) {
							...
							if (PrefabUtility.GetCorrespondingObjectFromSource(rootObject) != srcMesh && rootObject != srcMesh)
#else
							if (PrefabUtility.GetPrefabParent(rootObject) != srcMesh && rootObject != srcMesh)
#endif
							{
								if (!string.IsNullOrEmpty(AssetDatabase.GetAssetPath(srcMesh))) {
		...
		Dictionary<GameObject, GameObject> m_projectorPrefabs;
		...
		bool OnGUIAttachAutoProjectorToPrefabs()
		{
			...
			if (GUILayout.Button("Search for Prefab")) {
				...
				if (m_projectorPrefabs == null) {
					m_projectorPrefabs = new Dictionary<GameObject, GameObject>();
				}
...
#if UNITY_2018_2_OR_NEWER
							GameObject projectorPrefab = PrefabUtility.GetCorrespondingObjectFromSource(projector.gameObject) as GameObject;
#else
							GameObject projectorPrefab = PrefabUtility.GetPrefabParent(projector.gameObject) as GameObject;
#endif
							m_projectorPrefabs[projectorPrefab] = prefab;
						}
					}
					go.SetActive(false);
			...
			if (m_projectorPrefabs != null) {
				foreach (var keyValuePair in m_projectorPrefabs) {
					EditorGUILayout.BeginHorizontal();
					AutoProjector autoProjector = keyValuePair.Key.GetComponent<AutoProjector>();
					bool bAdded = autoProjector != null;
					if (!bAdded) {
						allProjectorsHaveAutoProjector = false;
					}
					EditorGUILayout.ObjectField(keyValuePair.Key, typeof(GameObject), true);
					GUILayout.Label("in");
					EditorGUILayout.ObjectField(keyValuePair.Value, typeof(GameObject), true);
					bool b = EditorGUILayout.Toggle(bAdded);
					if (b != bAdded) {
						if (b) {
							Undo.AddComponent<AutoProjector>(keyValuePair.Key);
						}
						else {
							Undo.DestroyObjectImmediate(autoProjector);
						}
						EditorUtility.SetDirty(keyValuePair.Key);
					}
					EditorGUILayout.EndHorizontal();
				}
