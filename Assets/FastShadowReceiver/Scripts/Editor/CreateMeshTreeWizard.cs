		...
		void Initialize(Transform meshTransform, LayerMask layers, MeshShadowReceiver receiver)
		{
			...
			if (m_srcMesh == null) {
				...
				if (m_meshes == null || m_meshes.Length == 0) {
					...
					GameObject prefabObject = PrefabUtility.GetCorrespondingObjectFromSource(m_rootObject) as GameObject;
#else
				...
				if (prefabObject != null) {
					...
					if (GUILayout.Button("Create Prefab")) {
						...
						if (!string.IsNullOrEmpty(path)) {
							...
							prefabObject = PrefabUtility.SaveAsPrefabAssetAndConnect(m_rootObject, path, InteractionMode.UserAction);
#else
